...
blp = Blueprint("Payment Accounts", __name__, description="Actions on payment accounts")


@blp.route("/payment/account")
class PaymentAccount(MethodView):
    ...
    def post(self, data):
        account = AccountModel.query.filter_by(
            ...
            account_category="Payment Account"
        ).first()
        if account:
        ...
        account = AccountModel(account_name=data["account_name"], account_number=data["account_number"],
                               account_description=data["account_description"], account_category="Payment Account")
        account.save_to_db()
        return account
    ...
    def get(self):
        accounts = AccountModel.query.filter_by(account_category="Payment Account").all()
        return accounts


@blp.route("/payment/account/<int:id>")
class PaymentAccountView(MethodView):
    @jwt_required(fresh=True)
    def delete(self, id):
        account = AccountModel.query.get_or_404(id)
        if account.account_category != "Payment Account":
